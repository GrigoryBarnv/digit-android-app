...
//		if (DEBUG) XLogWrapper.v(TAG, "requestPermission:device=" + device);
		...
		if (isRegistered()) {
			if (device != null) {
				...
				if (mUsbManager.hasPermission(device)) {
					// Avoid duplicate connect callbacks while an opened control block already exists.
					final UsbControlBlock existing = mCtrlBlocks.get(device);
					...
					try {
						...
						schedulePermissionGrantCheck(device, 12);
					} catch (final Exception e) {
	...
	private void schedulePermissionGrantCheck(final UsbDevice device, final int attemptsLeft) {
		if (device == null || attemptsLeft <= 0 || destroyed) {
			return;
		}
		mAsyncHandler.postDelayed(() -> {
			...
			schedulePermissionGrantCheck(device, attemptsLeft - 1);
		}, 500);
	}
