...
class CameraUVC(ctx: Context, device: UsbDevice) : MultiCameraClient.ICamera(ctx, device) {
    ...
    override fun closeCameraInternal() {
        ...
        mUvcCamera = null
        if (Utils.debugCamera) {
            Logger.i(TAG, " stop preview, name = ${device.serialNumber}")
        }
    }
    ...
    override fun setFps(fps: Int) {
        if (fps >= MIN_FS) {
            MAX_FPS = fps // Update the maximum FPS

            // Adjust camera parameters based on the new FPS

        } else {
            // Handle invalid FPS values
            Logger.e(TAG, "Invalid FPS value. Please provide a value greater than or equal to $MIN_FS.")
        }
    }
