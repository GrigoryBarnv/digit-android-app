...
abstract class CameraActivity: BaseActivity(), ICameraStateCallBack {
    ...
    /**
     * Get the serial number of the current connected UVC device.
     *
     * @return Serial number of the UVC device, or null if not available.
     */
    fun getSerialNumber(): String? {
        return getCurrentCamera()?.let { camera ->
            if (camera is CameraUVC) {
                camera.getUsbDevice()?.serialNumber
            } else {
                null
            }
        }
